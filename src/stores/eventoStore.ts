...
export const useEvento = create<PropEventoStore>((set) => ({
  ...
  filtroEstado: async (uf: string) => {
    ...
    await api.get(`/eventos/?uf=${uf}`).then((response) => {
      set({
        eventos: response.data.data,
      });
      console.log(uf);
  ...
  filtroCidade: async (cidade: string) => {
    ...
    await api.get(`/eventos?cidade=${cidade}`).then((response) => {
      set({
        eventos: response.data.data,
      });
    });
  ...
  filtroTipo: async (tipo: string) => {
    ...
    await api.get(`/eventos?tipo=${tipo}`).then((response) => {
      set({
        eventos: response.data.data,
      });
    });
  ...
  filtroData: async (data: string) => {
    ...
    await api.get(`/eventos?data=${data}`).then((response) => {
      set({
        eventos: response.data.data,
        copyEventos: response.data.data,
      });
  ...
  filtroNome: async (nome: string) => {
    set({
      eventos: useEvento
        .getState()
        .copyEventos.filter((evento: Evento) =>
